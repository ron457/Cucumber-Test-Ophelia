...
export class ShopPage {
  ...
  async continueCheckoutFailsafe() {
    this.log('Entering login details to attempt authenticated checkout');
    ...
    this.log('Login submitted, testing for valid session or backend rejection...');

    try {
      // Look for normal checkout button. Fast 5-second timeout.
      await expect(this.page.locator('[data-test="proceed-2"]')).toBeVisible({ timeout: 5000 });
      await this.page.locator('[data-test="proceed-2"]').click();
      this.log('proceed-2 clicked (authenticated flow succeeded)');
      await this.captureStep('continued checkout after authentication');
    } catch (err) {
      this.log('Backend rejected credentials or button missing! Engaging Guest Fallback...');
      
      // Clear alert banner or confirm error text exists
      await expect(this.page.getByText('Invalid email or password')).toBeVisible({ timeout: 5000 });
      this.log('Login error confirmed on UI');
      ...
      // Fill Guest Form Data
      await expect(this.page.locator('[data-test="guest-email"]')).toBeVisible({ timeout: 15000 });
      ...
      // Confirm and finalize guest session layout
      await expect(this.page.getByText(`Continuing as guest: ${testData.guest.firstName}`)).toBeVisible({ timeout: 15000 });
  ...
  async verifySuccess() {
    ...
    await expect(this.page.getByText('Thanks for your order! Your')).toBeVisible({ timeout: 30000 });
